...
def _add_activity(case_row, action: str, actor: str, details: str = ""):
    ...
    log = _parse_json(case_row.get("activity_log"))
    log.append({
        "id": len(log) + 1,
        "action": action,
        "actor": actor,
...
def _compute_urgency(case_row):
    ...
    hearings = _parse_json(case_row.get("hearings"))
    for h in hearings:
        try:
    ...
    if deadline:
        try:
            d_date = datetime.fromisoformat(deadline)
    ...
    if updated:
        try:
            ...
            if (now - last_update).days >= 7 and case_row.get("stage") != "Closed":
...
def _enrich_case(row):
    ...
    updates = _parse_json(row.get("daily_updates"))
    hearings = _parse_json(row.get("hearings"))
    notes = _parse_json(row.get("notes_log"))
    activity = _parse_json(row.get("activity_log"))
    ...
    return {
        **row,
        ...
        "stage": row.get("stage") or row.get("status", "Filed")
    }
...
async def create_case(data: CaseCreation, user: dict = Depends(get_current_user)):
    ...
    if user.get("role") != "admin":
        raise HTTPException(status_code=403, detail="Only the workspace administrator can assign new cases.")
    ...
    try:
        ...
        case_no = data.case_no or f"CASE-{int(datetime.now().timestamp())}"
...
async def add_hearing(case_id: int, data: HearingAdd, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        case = res.data[0]
        hearings = _parse_json(case.get("hearings"))
        ...
        activity_log = _add_activity(case, "hearing_added", user.get("name", "Unknown"),
            f"Hearing scheduled for {data.date} at {data.court or 'TBD'}")
...
async def get_hearings(case_id: int, user: dict = Depends(get_current_user)):
    ...
    try:
        res = supabase.table("lawyer_cases").select("hearings, title").eq("id", case_id).execute()
        if not res.data:
            raise HTTPException(status_code=404, detail="Case not found")
        hearings = _parse_json(res.data[0].get("hearings"))
...
async def add_note(case_id: int, data: NoteAdd, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        case = res.data[0]
        notes = _parse_json(case.get("notes_log"))
...
async def get_activity_log(case_id: int, user: dict = Depends(get_current_user)):
    ...
    try:
        res = supabase.table("lawyer_cases").select("activity_log, title, stage").eq("id", case_id).execute()
        if not res.data:
            raise HTTPException(status_code=404, detail="Case not found")
        
        activity = _parse_json(res.data[0].get("activity_log"))
        return {
            "case_id": case_id,
            "title": res.data[0].get("title"),
            "stage": res.data[0].get("stage"),
            "activity": activity,
            "total": len(activity)
        }
    except HTTPException:
...
async def get_lawyer_directory(user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        cases_res = supabase.table("lawyer_cases").select(
            "id, lawyer_email, title, case_no, stage, status, priority, deadline, case_type, court, client_name, updated_at, hearings, daily_updates, activity_log, notes_log"
        ).eq("firm_id", firm_id).execute()
...
async def get_lawyer_cases(user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        cases = [_enrich_case(row) for row in res.data]
        return {"cases": cases}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def submit_daily_update(case_id: int, data: DailyUpdateSubmission, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        case = res.data[0]
        existing_updates = _parse_json(case.get("daily_updates"))
        ...
        update_data = {
            "daily_updates": json.dumps(existing_updates),
            "activity_log": activity_log,
            "updated_at": now
        }
        if data.status:
            update_data["status"] = data.status
            update_data["stage"] = data.status
...
async def get_case_updates(case_id: int, user: dict = Depends(get_current_user)):
    try:
        res = supabase.table("lawyer_cases").select("daily_updates, title, status, stage").eq("id", case_id).execute()
        if not res.data:
            raise HTTPException(status_code=404, detail="Case not found")
        updates = _parse_json(res.data[0].get("daily_updates"))
        return {"case_id": case_id, "title": res.data[0].get("title"), "stage": res.data[0].get("stage"), "updates": updates, "total": len(updates)}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def remind_lawyer_case(case_id: int, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        supabase.table("lawyer_cases").update({"activity_log": activity_log}).eq("id", case_id).execute()
