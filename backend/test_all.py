...
def test_register():
    ...
    if r.status_code == 200:
        return "token" in data, f"[{r.status_code}] got token={bool(data.get('token'))}"
...
def test_login():
    ...
    data = r.json()
    return r.status_code == 200 and "token" in data, f"[{r.status_code}] got token={bool(data.get('token'))}"
...
def test_generate_draft():
    global draft_id
    r = requests.post(f"{BASE}/api/generate-draft", json={
...
def test_analyze_contract_bad_ext():
    ...
    with open(tmp, "w") as f:
        f.write("This is a text file, not a contract document.")
    with open(tmp, "rb") as f:
        r = requests.post(f"{BASE}/api/analyze-contract", files={"file": ("test.txt", f, "text/plain")})
    return r.status_code == 400, f"[{r.status_code}] correctly rejected .txt file"
